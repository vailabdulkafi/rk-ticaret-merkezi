...
const DeliveryMethodsSettings = () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(editingMethod ? 'Teslim şekli güncellendi' : 'Teslim şekli eklendi');
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['delivery-methods'] });
      toast.success('Teslim şekli silindi');
    },
    onError: (error) => {
      toast.error('Hata: ' + error.message);
    },
  ...
  return (
    <Card>
        ...
        <CardTitle className="flex items-center justify-between">
          ...
          <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>
                  {editingMethod ? 'Teslim Şeklini Düzenle' : 'Yeni Teslim Şekli'}
                </DialogTitle>
              </DialogHeader>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <Label htmlFor="name">Teslim Şekli Adı *</Label>
                  <Input
                    id="name"
                    value={formData.name}
                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    required
                    placeholder="Örn: Kargo, Kurye, Teslim Alma"
                  />
                </div>
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder="Teslim şekli hakkında detaylı bilgi"
                  />
                </div>
                <div>
      ...
      <CardContent>
        {methods?.length === 0 ? (
          <p className="text-gray-500 text-center py-4">Henüz teslim şekli eklenmemiş.</p>
        ) : (
